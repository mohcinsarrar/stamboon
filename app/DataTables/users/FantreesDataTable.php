...
class FantreesDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('actions', 'users.fantree.actions')
            ->editColumn('name', function(Fantree $model) {
            ...
            ->editColumn('generation', function(Fantree $model) {
                $generation = 0;
                if($model->stats != null){
            ...
            ->editColumn('indis', function(Fantree $model) {
                if($model->stats != null){
            ...
            ->editColumn('print_number', function(Fantree $model) {
    ...
    public function html(): HtmlBuilder
    {
        ...
        return $this->builder()
                    ...
                    ->orderBy(1)
                    ->buttons([
                        Button::raw('add_fantree')
                        ->className('mx-2 btn btn-primary rounded')
                        ->attr([
                            'data-bs-toggle'=>'offcanvas',
                            'data-bs-target'=> '#offcanvasAddFantree'
                            ])
                        ->text('<i class="ti ti-plus me-0 me-sm-1 ti-xs"></i><span class="d-none d-sm-inline-block">Add Fanchart</span>')
                    ]);
