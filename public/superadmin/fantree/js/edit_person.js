...
function edit_person() {
    ...
    // fill person id
    formUpdatePerson.querySelector('.person_id').value = personInfo.id

    // activate sex if root person
    if(personInfo.order == 0){
        formUpdatePerson.querySelector('#sex_container').classList.remove('d-none');
        if(personInfo.gender == 'M'){
            formUpdatePerson.querySelector('#sex_container #male').checked = true;
        }
        else{
            formUpdatePerson.querySelector('#sex_container #female').checked = true;
        }
    }
    else{
        formUpdatePerson.querySelector('#sex_container').classList.add('d-none');
    }
    ...
    if (personInfo.birth != null) {
        //
        var birthDate = parseDateGlobal(personInfo.birth,target_format=treeConfiguration.default_date, target_date_style = 'number', target_separator = "-",  date_style = 'string', separator = ' ')
        formUpdatePerson.querySelector('.birth_date').value = birthDate
    }

    // change date msg

    formUpdatePerson.querySelector('#date-msg span').innerHTML = "Date format : "+treeConfiguration.default_date+" or YYYY"
    ...
    if (personInfo.status == 'Deceased') {
        ...
        if (personInfo.death != null) {
            ...
            var deathDate = parseDateGlobal(personInfo.death,target_format=treeConfiguration.default_date, target_date_style = 'number', target_separator = "-",  date_style = 'string', separator = ' ');
...
document.getElementById('formUpdatePerson').addEventListener('submit', function (event) {
    ...
    if(treeConfiguration.default_date == 'YYYY-MM-DD'){
        var regexFullDate = /^\d{4}-\d{2}-\d{2}$/;
    }
    else if(treeConfiguration.default_date == 'MM-DD-YYYY'){
        var regexFullDate = /^\d{2}-\d{2}-\d{4}$/;
    }
    else if(treeConfiguration.default_date == 'DD-MM-YYYY'){
        var regexFullDate = /^\d{2}-\d{2}-\d{4}$/;
    }
    ...
    $.ajax({
        url: "/superadmin/fantree/update/"+fantree_id,
        type: 'POST',
        data: formData,
        encode: true,
        dataType: 'json',
