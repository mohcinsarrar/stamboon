...
function edit_image() {
    ...
    if (personInfo.photo != undefined) {
        image.src = "/storage/portraits_fantree/" + personInfo.photo;
...
document.getElementById('save_image').addEventListener('click', function () {
    ...
    imageData.person_id = personInfo.id;

    // create an xhr request
    let request = new XMLHttpRequest();

    let fantree_id = get_fantree_id()
    // open the request to the "/superadmin/fantree/saveimage" url
    request.open("POST", "/superadmin/fantree/saveimage/"+fantree_id, true);
...
save_image_placeholder.addEventListener('click', function () {
    ...
    let fantree_id = get_fantree_id()
    $.ajax({
        url: "/superadmin/fantree/saveimage/"+fantree_id,
        type: 'POST',
        data: {
